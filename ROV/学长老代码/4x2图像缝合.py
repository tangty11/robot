...
def board(pano):
    ...
    #cv2.copyMakeBorder(src,top,bottom,left,right,borderType,value)填充图像：原图像，四边，边界类型
...
#上下图片分开缝合形成两个半张的图。是将上下四张分开存了，然后将结果存两张：上半张和下半张，再在后面对其旋转，左右缝合
i=0
for i in range(1):
    path = 'D:\\testpicture'+str(i)#上半张为testpicture0，下半张为....1
    files = os.listdir(path)
    files.reverse()
    img_right = cv2.imread(path+'\\'+files.pop(0))
    for file in files:
        img_left = cv2.imread(path+'\\'+file)
        ...
        vis = OptimizeSean(img_left, trans, vis)  # 平滑缝合部分
        # vis = board(vis)  # 去除图像边缘黑色部分，使用之前记得去函数体里看看膨胀处理的卷积核和迭代次数是否合适，否则有一定可能出一点小问题
        cv2.imwrite(path + '\\r' + file, vis)
        img_right = copy.deepcopy(vis)

    vis = board(vis)  # 去除图像边缘黑色部分，使用之前记得去函数体里看看膨胀处理的卷积核和迭代次数是否合适，否则有一定可能出一点小问题
    cv2.imwrite('D：\\result\result'+str(i)+'jpg', vis)#储存上下半张
...
#读取上下两边图像旋转后左右缝合
img0 = cv2.imread('D：\\result\result0.jpg')
img1 = cv2.imread('D：\\result\result1.jpg')
img0 = cv2.resize(img, None, fx=0.3, fy=0.3)
img1 = cv2.resize(img, None, fx=0.3, fy=0.3)

#########旋转图像便于左右缝合........注意上下不要混了
# 上边半侧逆时针旋转放在左边
img_left = np.rot90(img0)
# 下半边逆时针放右边
img_right = np.rot90(img1)
...
cv2.imwrite('D：\\result\finalresult', vis)#存个最终图片，本应该旋转一下但是偷懒了
plt.xticks([]), plt.yticks([])
plt.imshow(cv2.cvtColor(vis, cv2.COLOR_BGR2RGB))
plt.show()